...
-- Alumno: MONTIEL ASENSIO LUCÍA
-- Fecha de entrega:  DIA | MES | AÑO
--
-- Relación de Ejercicios 1. Ejercicios resueltos: 5, 7, 10, 14, 17 
-- 
-------------------------------------------------------------------------------
...
descomponer x = (horas, minutos, segundos)
   where
     (horas, resto1)      = divMod x 3600
     (minutos, segundos)   = divMod resto1  60
...
--p_descomponer x = x>=0 ==> h*3600 + m*60 + s == x && m `entre` (0,59) && s `entre` (0,59) where (h,m,s) = descomponer x
...
raices x y z 
    | dis< 0 = error "Raices no reales"
    | otherwise = ((-y + raiz) / denominador, (-y - raiz) / denominador) 
    where
        dis = y^2 - 4*x*z
        raiz = sqrt dis
        denominador = 2*x
...
--10b)
p1_raíces :: Double -> Double -> Double -> Bool
...
potencia' :: Integer -> Integer -> Integer
potencia' b n 
    | (mod n 2) == 0 = basepar*basepar 
    | n < 0 = error "Exponente negativo"
    | n == 0 = 1
    | n == 1 = b
    | otherwise = b*baseimpar*baseimpar
    where 
        basepar = potencia' b (div n 2)
        baseimpar = potencia' b (div (n-1) 2)
